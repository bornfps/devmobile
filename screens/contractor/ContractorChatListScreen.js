...
export default function ContractorChatListScreen({ navigation }) {
  const { user } = useAuth();
  ...
  return (
    <ScreenContainer>
      ...
      {myConversations.length === 0 ? (
        <EmptyState message="Nenhuma conversa iniciada ainda." />
      ) : (
        myConversations.map((conversation) => {
          ...
          return (
            <Pressable
              key={conversation.id}
              style={styles.card}
              onPress={() => navigation.navigate('ContractorChat', { conversationId: conversation.id })}
            >
              <Text style={styles.name}>{other?.name || 'Participante'}</Text>
              <Text style={styles.meta}>{request?.serviceName || 'Servico'}</Text>
